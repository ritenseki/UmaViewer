...
数据来源：LiveTimelineDefine.cs 枚举 + WorkSheet CS字段核查 + worksheet_fields.txt（58首歌扫描）+ Director/Control代码审查
...
| 9 | BgColor2 ★(1) | 背景渐变/环境色（两色混合） | 🔴 Bug已修 | ~~WorkSheet有字段+event已声明，但AlterLateUpdate从未调用AlterUpdate_BgColor2，Director未订阅~~ → 2026-05-07已修复 |
| 10 | MonitorControl ★(2) | LED舞台屏幕内容控制 | ❌ | — |
| 11 | CameraSwitcher | 切换活动摄像机 | ✅ | — |
| 12 | LipSync | 口型同步 | ✅ | — |
| 13 | PostEffectDOF | 景深模糊（单机） | ❌ | — |
| 14 | PostEffectBloomDiffusion | 泛光扩散（单机） | ❌ | — |
| 15 | RadialBlur | 径向运动模糊（单机） | ❌ | — |
...
| 38 | HdrBloom | HDR泛光强度/阈值 | ⚠️ | 字段映射未经bundle验证（30首歌均无数据）；bloomIntensity/threshold映射可能有误 |
| 39 | PostFilm | 全屏后期叠加（晕影/颜色/UV电影） | ❌ | LiveTimelineKeyPostFilmData字段存在，WorkSheet无postFilmKeys声明 |
| 40 | Fade | 场景淡入淡出 | ❌ | — |
...
| 57 | LensFlare | 镜头光晕 | ❌ | — |
| 58 | Environment ★(1) | 舞台环境参数（天空盒/环境光） | ❌ | WorkSheet无environmentDataLists字段 |
...
| 72 | CharaFootLight | 角色脚部补光灯 | ❌ | — |
| 73 | ChromaticAberration ★(1) | 色差后处理 | ⚠️ | redOffset/greenOffset/blueOffset（通道偏移）无URP内置对应；effectType未使用 |
| 74 | LightProjection ★(18!) | 投影灯（Gobo图案灯） | ❌ | WorkSheet无lightProjectionList字段；song 1001有18条，优先级极高 |
| 75 | Billboard | 广告牌（始终面向摄像机的面片） | ❌ | — |
...
| ID | 名称 | 作用 | 状态 | Bug |
|----|------|------|------|-----|
| 76 | MultiCameraPostFilm ★(2) | 多机位全屏后期叠加 | ❌ | WorkSheet无postFilm1MultiCameraKeys字段 |
...
| ID | 名称 | 作用 | 状态 | Bug |
|----|------|------|------|-----|
| 82 | AdditionalLight | 附加动态光源 | ❌ | — |
| 83 | MultiLightShadow | 多光源阴影 | ❌ | — |
...
## song 1001 有数据但未实现的优先级排序

LightProjection(74, 18条) > Audience(34, 12条) > MobControl(84, 4条) > CyalumeControl(85, 4条) > Projector(17, 4条) > MultiCameraPostFilm(76, 2条) > MultiCameraBloom(77, 2条) > NodeScale(70, 2条) > Environment(58, 1条) > SweatLocator(59, 3条) > MonitorControl(10, 2条) > Animation(29, 1条)
