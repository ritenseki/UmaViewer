...
| BgColor2 (9) | ⚠️ 接线完成，`RenderSettings.ambient*` 打错目标（见卡点） |
...
| 轨道 | 不明白什么 |
|------|-----------|
| BgColor2 (9) | entry 名 `BgWashA`～`J` 在所有 bundle 里找不到；舞台 shader 不响应 `RenderSettings.ambient`（15/58 首）|
...
| `Assets/Scripts/umamusume/Gallop/Live/StageController.cs` | StageObjectMap（按名字查舞台对象）|
